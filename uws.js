...
const REGISTER_ERROR = 'Registering more than one listener to a WebSocket is not supported.';
const DEFAULT_PAYLOAD_LIMIT = 16777216;
const PERMESSAGE_DEFLATE = 1;
const SLIDING_DEFLATE_WINDOW = 16;
const OPCODE_TEXT = 1;
const OPCODE_BINARY = 2;
const OPCODE_PING = 9;

let internalUpgradeReq = null;
...
function emitConnection(ws) {
    this.emit('connection', ws, internalUpgradeReq);
}
...
native.client.group.onDisconnection(clientGroup, (external, code, reason, webSocket) => {
    webSocket.external = null;

    process.nextTick(() => {
        webSocket.internalOnClose(code, reason);
    });
...
class WebSocket {
    constructor(external) {
        ...
        this.internalOnPing = noop;
        this.internalOnPong = noop;
    }

    get upgradeReq() {
        return internalUpgradeReq;
    }

    get ready() {
        return this.external !== undefined && this.external !== null;
    }

    get address() {
        const addr = this.external ? native.getAddress(this.external) : new Array(3);
        return {
            port: addr[0],
            address: addr[1],
            family: addr[2]
    ...
    emit(eventName, arg1, arg2) {
        if (eventName === 'message') {
            ...
            this.internalOnPing(arg1);
        } else if (eventName === 'pong') {
            this.internalOnPong(arg1);
        }
        return this;
    }
    ...
    on(eventName, f) {
        if (eventName === 'message') {
            ...
            if (this.internalOnPing !== noop) {
                throw Error(REGISTER_ERROR);
            }
            this.internalOnPing = f;
        } else if (eventName === 'pong') {
            if (this.internalOnPong !== noop) {
                throw Error(REGISTER_ERROR);
            }
            this.internalOnPong = f;
        }
        return this;
    }
    ...
    removeListener(eventName, cb) {
        if (eventName === 'message' && this.internalOnMessage === cb) {
            ...
            this.internalOnClose = noop;
        } else if (eventName === 'ping' && this.internalOnPing === cb) {
            this.internalOnPing = noop;
        } else if (eventName === 'pong' && this.internalOnPong === cb) {
            this.internalOnPong = noop;
        }
        return this;
    }
    ...
    send(message, cb, binary, compress) {
        if (this.external) {
            const bin = binary && typeof binary === 'boolean' ? binary : typeof message !== 'string';

            native.server.send(this.external, message, bin ? OPCODE_BINARY : OPCODE_TEXT, cb ? (() => {
                process.nextTick(cb);
            }) : undefined, compress);
        } else if (cb) {
    ...
    ping(message, options, dontFailWhenClosed) {
        if (this.external) {
            native.server.send(this.external, message, OPCODE_PING, false);
        }
    }
...
class WebSocketClient extends WebSocket {
    ...
    send(message, cb, binary, compress) {
        if (this.external) {
            ...
            native.client.send(this.external, message, bin ? OPCODE_BINARY : OPCODE_TEXT, cb ? (() => {
                process.nextTick(cb);
            }) : undefined, compress);
        } else if (cb) {
            cb(new Error('not opened'));
    ...
    ping(message, options, dontFailWhenClosed) {
        if (this.external) {
            native.client.send(this.external, message, OPCODE_PING, false);
        }
    }
...
class WebSocketServer extends EventEmitter {
    constructor(options, callback) {
        ...
        if (!options.noServer) {
                ...
                if (!options.path || options.path == request.url.split('?')[0].split('#')[0]) {
                    if (options.verifyClient) {
                        const info = {
                            ...
                            secure: !!(request.connection instanceof tls.TLSSocket && (request.connection.authorized || request.connection.encrypted))
                        };
            ...
            this.httpServer.on('error', (err) => {
                if (options.portAutoIncTo && err && (err.code === "EADDRINUSE" || err.errno === "EADDRINUSE")) {
                    options.port++;
                    if (options.port <= options.portAutoIncTo) {
        ...
        native.server.group.onDisconnection(this.serverGroup, (external, code, reason, webSocket) => {
            webSocket.external = null;
        ...
        native.server.group.onConnection(this.serverGroup, (external) => {
            ...
            internalUpgradeReq = null;
        });

        if (options.port !== undefined) {
            this.httpServer.allowHalfOpen = options.allowHalfOpen !== undefined ? options.allowHalfOpen : false;
            this.httpServer.listen(options, () => {
                this.emit('listening');
                callback && callback();
            });
    ...
    broadcast(message, binary) {
        if (this.serverGroup) {
            const bin = binary && typeof binary === 'boolean' ? binary : typeof message !== 'string';
            native.server.group.broadcast(this.serverGroup, message, bin);
        }
    }
